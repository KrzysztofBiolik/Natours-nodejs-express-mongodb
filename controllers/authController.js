...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true, // cookie nie mogą być dostępne lub zmodyfikowane w przeglądarce
  };
  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true;
  // only send with https request
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  (user.passwordResetToken = undefined),
    (user.passwordResetExpired = undefined);
  // to tylko modyfikuje dokument, ale nie zapisuje go
  await user.save();

  // 3 UpdatechangePasswordAt property for the user
